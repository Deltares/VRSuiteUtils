...
def extract_profile_data(profile_data, window, titel, path):
    ...
    slopes = (
        df[(df["z_covariance"] >= 0.15) & (df["x"] >= -50) & (df["x"] <= 50)]
        .reset_index()
    ...
    if matched_pivot_index == 0:
        inner_crest_index = int(
            slopes.iloc[pivot_index["slope index"].values[0] - 1]["profile index"]
        )
        ...
        if len(pivot_index) == 1:
            ...
            outer_toe_index = int(
                slopes.iloc[
                    int(pivot_index.iloc[matched_pivot_index + 1]["slope index"] - 1)
                ]["profile index"]
            )
    ...
    # In case of a berm
        ...
        berm_inner_toe_side_index = int(
            slopes.iloc[
                int(pivot_index.iloc[matched_pivot_index - 1]["slope index"] - 1)
            ]["profile index"]
        )
        berm_inner_crest_side_index = int(
            pivot_index.iloc[matched_pivot_index - 1]["profile index"]
        )
        ...
        if matched_pivot_index - 1 == pivot_index.index[0]:
            ...
            inner_toe_index = int(
                pivot_index.iloc[matched_pivot_index - 2]["profile index"]
            )
    ...
    if matched_pivot_index == 0:
        ...
        z_coords = [
            ...
            (df.loc[inner_crest_index]["z"] + df.loc[outer_crest_index]["z"]) / 2,
            (df.loc[inner_crest_index]["z"] + df.loc[outer_crest_index]["z"]) / 2,
...
def main():
    ...
    output_path = Path(r"..\..\data\base_files\ProfilesFromAHN").joinpath(
        traject, "profiles"
    )
    ...
    traject_data = pd.read_excel(
        input_path.parent.joinpath((output_filename)), header=1
    )
    traject_data = traject_data[
        (traject_data["Traject"] == traject) & (traject_data.iloc[:, 5])
    ]
    ...
    for i in index:
        ...
        profile_number = "Dwarsprofiel_" + str(number[0]).zfill(
            len(str(index.iloc[-1]))
        )  # if not extension else 'Dwarsprofiel_' + str(number[0]).zfill(len(str(index.iloc[-1]))) + str(extension[0])
        profile_data = (
            input_data[input_data["dijkvaknummer"] == int(number[0])]
            .reset_index(drop=True)
            .rename(
                columns={
                    "afstand buk [m, buitenkant +]": "x",
                    "z_ahn [m NAP]": "z",
                    "x": "x_coord",
                    "y": "y_coord",
                }
        ...
        profile = extract_profile_data(
            profile_data, window=2, titel=profile_number, path=output_path
        )
