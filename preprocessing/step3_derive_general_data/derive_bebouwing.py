...
teenvakken_gdf = gpd.GeoDataFrame(columns=["vaknaam", "geometry"])
# teenvakken_gdf = teenvakken_gdf.append({"vaknaam": vakindeling_gdf.vaknaam[0], "geometry": line}, ignore_index=True)
# replace the above line with pd.concat.
...
for i in range(2, len(splits)):
    ...
    # teenvakken_gdf = teenvakken_gdf.append({"vaknaam": vakindeling_gdf.vaknaam[i], "geometry": line}, ignore_index=True)
    # rewrite the above line, but use pd.concat instead of append. Don't forget to pass an index
...
buildings_gdf.to_file(intermediate_dir.joinpath("buildings_traject{0}.geojson".format(traject_name)), driver='GeoJSON')
...
# save to csv
building_matrix.to_csv(intermediate_dir.joinpath("building_count_traject{0}.csv".format(traject_name)))