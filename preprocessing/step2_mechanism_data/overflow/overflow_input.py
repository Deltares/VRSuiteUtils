...
class OverflowInput:
    ...
    def get_HRLocation(db_location,hring_data):
        ...
        for count, line in hring_data.iterrows():
            hring_data.loc[count,'HRLocation'] = locs.loc[locs['Name'] == line['hr_koppel']]['HRDLocationId'].values[0]
    ...
    def select_locs(self):
        ...
        self.hring_data = self.hring_data.iloc[indices].reset_index().rename(columns={'index': 'ID'}).drop_duplicates(subset=['ID'])

    def verify_and_filter_columns(self):
        required_cols = ['doorsnede','bovengrens','ondergrens','prfl_bestand','orientatie','dijkhoogte','zodeklasse','bovengrens_golfhoogteklasse','kruindaling','HRLocation']
