...
import numpy as np
import pandas as pd
from project_utils.readSteentoetsFile import read_steentoets_file
from scipy.special import ndtri
import matplotlib.pyplot as plt
from project_utils.DiKErnel import write_JSON_to_file, read_JSON
from project_utils.functions_integrate import issteen
from pathlib import Path

# paths
input_dir = Path(r'c:\VRM\test_revetments')
steentoets_dir = Path(r'c:\VRM\test_revetments\steentoets')
figures_ZST = Path(r'c:\VRM\test_revetments\figures_ZST')
output_dir = Path(r'c:\VRM\test_revetments\output')

def ZST_get_relation():
# read revetment file
df = pd.read_csv(input_dir.joinpath('Bekleding_default.csv'),
                 usecols=['vaknaam', 'dwarsprofiel', 'signaleringswaarde', 'ondergrens', 'faalkansbijdrage',
                          'lengte_effectfactor', 'locationid', 'steentoetsfile', 'prfl', 'qvar_p1', 'qvar_p2',
...
indexvak = [0, 1, 2] # can we change this to the vaknaam?
...
for index in indexvak:
    ...
    Zo, Zb, overgang, tana, Bsegment, toplaagtype, D, rho_s, Hs_ini, overschot, delta, D_voldoet, ratio_voldoet = read_steentoets_file(steentoets_dir.joinpath(steentoetsFile), dwarsprofiel)
    ...
    for i in range(0, len(evaluateYears)):
        ...
        write_JSON_to_file(data, output_dir.joinpath("ZST_{}_{}.json".format(index, evaluateYears[i])))
