...
def check_traject_shape(traject_name, shape):
    ...
    if "TRAJECT_ID" not in shape.columns:
        raise Exception("Traject shape does not have field TRAJECT_ID")
    if shape["TRAJECT_ID"].unique().shape[0] != 1:
        raise Exception("Traject shape does not have unique TRAJECT_ID")
    TRAJECT_ID = shape["TRAJECT_ID"].unique()
    crs = shape.crs

    # check 2: ensure that it is the correct traject (TRAJECTID)
    if TRAJECT_ID[0] != traject_name:
        raise Exception(
            "Traject shape does not match traject_name: shape has {}, while it should be {}".format(
                TRAJECT_ID[0], traject_name
            )
        )
    ...
    if shape.shape[0] == 1:
        ...
        print(
            "Multiple sections detected in traject shape, welding to 1 single traject"
        )
        new_geometry = ops.linemerge(geometry.MultiLineString(list(shape.geometry)))
        updated_shape = gpd.GeoDataFrame(
            data=TRAJECT_ID, columns=["TRAJECT_ID"], geometry=[new_geometry]
        )
...
def check_vakindeling_algemeen(df_vakindeling, traject_length):
    ...
    if any(df_vakindeling["NUMMER"].duplicated()):
        raise ("values in NUMMER are not unique")
    ...
    for mechanism in df_vakindeling.columns[
        df_vakindeling.columns.str.contains("DOORSNEDE")
    ]:
        if all(df_vakindeling[mechanism].isna()):
            warnings.warn(
                "WARNING: No mechanism data for {}".format(mechanism.split("_")[1]),
                ImportWarning,
            )
        elif any(df_vakindeling[mechanism].isna()):
            warnings.warn(
                "WARNING: Some sections do not have input for {}".format(
                    mechanism.split("_")[1]
                ),
                ImportWarning,
            )
    ...
    if any(np.diff(df_vakindeling.MEAS_END) < 0):
        raise ValueError(
            "MEAS_END values are not always increasing if sorted on OBJECTID"
        )
    if any(np.diff(df_vakindeling.MEAS_START) < 0):
        raise ValueError(
            "MEAS_START values are not always increasing if sorted on OBJECTID"
        )
    if any(np.subtract(df_vakindeling.MEAS_END, df_vakindeling.MEAS_START) < 0.0):
        raise ValueError("MEAS_START is higher than MEAS_END for at least 1 section")

    # check if MEAS_END.max() is approx equal to traject_length
    np.testing.assert_approx_equal(
        df_vakindeling.MEAS_END.max(), traject_length, significant=5
    )
...
def findWeakestGEKB(
    ...
    GEKB_shape,
    db_dir=Path(
        r"n:\Projects\11208000\11208392\C. Report - advise\Invoer & vakindeling\Invoer & vakindeling\test_38_1"
    ),
):
    indices = []
    if len(GEKB_shape.TRAJECT_ID.unique()) > 1:
        raise Exception("Shape contains multiple trajects")
    traject_id = GEKB_shape.TRAJECT_ID.unique()[0]
    # first get middle of sections (we assume the cs is at that spot)
    GEKB_shape["M_MID"] = np.average(
        np.vstack([GEKB_shape.M_VAN.values, GEKB_shape.M_TOT.values]).T, axis=1
    )
    for count, vak in vakindeling.iterrows():
        # get subset of GEKB locations
        subset = GEKB_shape.loc[
            (GEKB_shape.M_MID > vak.MEAS_START) & (GEKB_shape.M_MID < vak.MEAS_END)
        ]
        ...
        if line.shape[0] == 0:
            ...
            if line.shape[0] != 1:
                warnings.warn(
                    "Multiple cross sections have highest P_f for section {}".format(
                        vak.NUMMER
                    )
                )
            # write index to list
    ...
    GEKB_selectie = (
        GEKB_shape.iloc[indices].reset_index().rename(columns={"index": "GEKB_id"})
    )
    ...
    for count, line in GEKB_selectie.iterrows():
        loc_ids.append(
            locs.loc[locs["Name"] == line["HR koppel"]]["HRDLocationId"].values[0]
        )
...
def write_intermediate_data(traject_name, vakindeling_tabel_path, clear_data=False):
    ...
    traject_shape = get_traject_shape_from_NBPW(
        "38-1",
        NBWP_shape_path=Path(
            r"c:\Users\klerk_wj\OneDrive - Stichting Deltares\00_Projecten\11_VR_Prioritering WSRL\Gegevens 38-1\BasisbestandWaterkeringen\dijktrajecten.shp"
        ),
    )
    ...
    # save traject_shape
    traject_shape.to_file(
        intermediate_dir.joinpath("Traject {}.shp".format(traject_name))
    )
    # read the vakindeling:
    vakindeling_df = pd.read_excel(
        vakindeling_tabel_path,
        sheet_name="Algemeen",
        dtype={
            "OBJECTID": np.int32,
            "NUMMER": str,
            ...
            "VAN_DP": str,
            "TOT_DP": str,
            "MEAS_START": np.float32,
            "MEAS_END": np.float32,
            "VAKLENGTE": np.float32,
            "IN_ANALYSE": bool,
        },
    )
    ...
    raw_GEKB_data = pd.read_excel(
        vakindeling_tabel_path.parent.joinpath(
            "Overslag", "Bijlage A DATA-HB_TRAJECT-38-1_v20211117.xlsx"
        ),
        sheet_name="Per dijkpaal",
        header=15,
        usecols=relevante_kolommen,
    )
    # renaming some unclear headers:
    raw_GEKB_data = raw_GEKB_data.rename(
        columns={"Column171": "Golfhoogteklasse", "Column182": "Faalkans"}
    )
    # replace non-numeric values (NVT means not applicable so Pf=0)
    raw_GEKB_data["Faalkans"] = raw_GEKB_data["Faalkans"].replace({"NVT": 0.0})
    raw_GEKB_data["Faalkans"] = np.divide(
        1.0, raw_GEKB_data["Faalkans"].astype(np.float64)
    )
    ...
    if len(glob.glob(str(input_dir.joinpath("Overslag", "Vakindeling", "*.shp")))) > 1:
        raise Exception("Multiple shapefiles for GEKB detected.")
    gekb_shp_path = Path(
        glob.glob(str(input_dir.joinpath("Overslag", "Vakindeling", "*.shp")))[0]
    )
    GEKB_shape = gpd.read_file(gekb_shp_path)
    GEKB_shape = (
        GEKB_shape.merge(raw_GEKB_data, left_on="Vaknaam", right_on="dijkvak")
        .sort_values("M_VAN")
        .drop("OBJECTID", axis=1)
        .reset_index(drop=True)
        .reset_index()
        .rename(columns={"index": "OBJECTID"})
    )  # now we select the cross sections to obtain a filtered dataset
    GEKB_data, vakindeling_gdf = findWeakestGEKB(
        vakindeling_gdf, GEKB_shape, db_dir=input_dir.joinpath("Overslag")
    )
    ...
    if input_dir.joinpath("Overslag", "GEKB_data.csv").exists():
        old_GEKB_data = pd.read_csv(
            input_dir.joinpath("Overslag", "GEKB_data.csv"), index_col=0
        )
        # compare:
    ...
    # STBI data
    STBI_data = pd.read_excel(vakindeling_tabel_path, sheet_name="STBI")[
        ["DOORSNEDE", "BEREKENING", "SCENARIOKANS", "SF", "BETA", "STIX"]
    ]
    STBI_data = check_STBI_data(STBI_data)
    # profielen
    raw_profile_data = pd.read_excel(
        input_dir.joinpath("Dijkprofielen", "GEOMETRY_TEMPLATE_v3_38_1_STBI.xlsx"),
        sheet_name="KARAKTERISTIEKE_PUNTEN",
        header=9,
    )
    # TODO improve CS_NUM (e.g. by a table that reference DP and M_value)
    raw_profile_data["CS_NUM"] = raw_profile_data["ID_GEOMETRY_LOCATION"].map(
        lambda x: dp_as_number(x)
    )
    profiles = []
    for count, vak in vakindeling_gdf.iterrows():
        if vak.IN_ANALYSE:
            ...
            if any(raw_profile_data.ID_GEOMETRY_LOCATION.isin([vak.DSN_STBI])):
                if (
                    len(
                        raw_profile_data.loc[
                            raw_profile_data.ID_GEOMETRY_LOCATION == vak.DSN_STBI
                        ]["ID_GEOMETRIE"]
                    )
                    > 1
                ):
                    raise Exception("Multiple profiles found")
                profiles.append(
                    raw_profile_data.loc[
                        raw_profile_data.ID_GEOMETRY_LOCATION == vak.DSN_STBI
                    ]["ID_GEOMETRIE"].values[0]
                )
            ...
            # NB: dit is een custom stap die weg kan als we CS_NUM goed hebben
                ...
                profiles.append(
                    raw_profile_data.loc[
                        raw_profile_data.ID_GEOMETRY_LOCATION == vak.DSN_STBI[:-2]
                    ]["ID_GEOMETRIE"].values[0]
                )
            ...
            # ANDERS: Zoek op basis van getalswaarde
                ...
                warnings.warn(
                    "Voor dijkvak {} is geen profiel gevonden, beste schatting is gebruikt".format(
                        vak.NUMMER
                    )
                )
                options = raw_profile_data.loc[
                    (raw_profile_data.CS_NUM > vak.MEAS_START)
                    & (raw_profile_data.CS_NUM < vak.MEAS_END)
                ]
                if len(options) > 1:
                    raise Exception(
                        "Meerdere mogelijke profielen voor dijkvak {}".format(
                            vak.NUMMER
                        )
                    )
                elif len(options) == 0:
                    ...
                    idx_min = np.argmin(
                        np.abs(np.subtract(raw_profile_data.CS_NUM, midpoint))
                    )
                    profiles.append(
                        raw_profile_data.reset_index(drop=True).iloc[idx_min][
                            "ID_GEOMETRIE"
                        ]
                    )
                    warnings.warn(
                        "Geen geschikt profiel gekozen. Dichtsbijzijnde profiel in ander vak genomen ({})".format(
                            profiles[-1]
                        )
                    )
                else:
        ...
        for char_point in char_points:
            if char_point == "BUT":  # custom statement, has to be removed later on.
                if np.isnan(profile_data["GEO_KAR_X_BBB"].item()):  # geen buitenberm
                    xz.append(
                        (
                            profile_data["GEO_KAR_X_{}".format(char_point)].item(),
                            profile_data["GEO_KAR_Z_{}".format(char_point)].item(),
                        )
                    )
                else:
                    xz.append(
                        (
                            profile_data["GEO_KAR_X_BBB"].item(),
                            profile_data["GEO_KAR_Z_BBB"].item(),
                        )
                    )
            else:
                xz.append(
    ...
    # Maatregelen
    shutil.copytree(
        input_dir.joinpath("Maatregelen"), intermediate_dir.joinpath("Maatregelen")
    )

    # Overslag/Resultaten
    shutil.copytree(
        input_dir.joinpath("Overslag", "Resultaten_W2100"),
        intermediate_dir.joinpath("Overslag", "Resultaten_W2100"),
    )
    shutil.copytree(
        input_dir.joinpath("Overslag", "Resultaten_WBI2017"),
        intermediate_dir.joinpath("Overslag", "Resultaten_WBI2017"),
    )

    # Waterstand/Resultaten
    shutil.copytree(
        input_dir.joinpath("Waterstand", "Resultaten_W2100"),
        intermediate_dir.joinpath("Waterstand", "Resultaten_W2100"),
    )
    shutil.copytree(
        input_dir.joinpath("Waterstand", "Resultaten_WBI2017"),
        intermediate_dir.joinpath("Waterstand", "Resultaten_WBI2017"),
    )

    # Data voor mechanismen:
    write_df_to_csv(
        GEKB_data_reduced, intermediate_dir.joinpath("Overslag", "GEKB_data.csv")
    )
    ...
    # Bebouwing
    write_df_to_csv(
        bebouwing, intermediate_dir.joinpath("Bebouwing", "Bebouwing_data.csv")
    )

    # Vakindeling:
    vakindeling_gdf.to_file(
        intermediate_dir.joinpath("Vakindeling_Veiligheidsrendement.shp")
    )


def write_dike_section_files_legacy(
    working_dir, vakindeling_gdf, housing_data, measureset_data, gekb_data
):
    ...
    for count, section in vakindeling_gdf.iterrows():
        ...
        general_values = [
            ...
            section["DSN_STBI"],
            section["DSN_STPH"],
            "Waterstand_" + section["DSN_GEKB"],
            "Waterstand_" + section["DSN_GEKB"],
            gekb_data.loc[gekb_data["Vaknaam"] == section["DSN_GEKB"]][
                "Kruindaling"
            ].item(),
            gekb_data.loc[gekb_data["Vaknaam"] == section["DSN_GEKB"]][
                "prfl_dijkhoogte"
        ...
        # DikeProfile
        DikeProfile = pd.read_csv(
            working_dir.joinpath(
                "intermediate", "Profielen", section["DIJKPROFIE"] + ".csv"
            ),
            index_col=0,
        )
        ...
        # Housing
        Housing = (
            housing_data.loc[housing_data.OBJECTID == section.OBJECTID]
            .drop(columns=["OBJECTID", "NUMMER"])
            .transpose()
            .reset_index()
        )
        ...
        if len(section["NUMMER"]) == 1:
            ...
            writer = pd.ExcelWriter(
                working_dir.joinpath(
                    "output", "DV{:02d}.xlsx".format(np.int32(section["NUMMER"]))
                )
            )
        else:
            ...
            writer = pd.ExcelWriter(
                working_dir.joinpath("output", "DV{}.xlsx".format(section["NUMMER"]))
            )
...
def write_mechanism_data_legacy(
    working_dir, vakindeling_gdf, housing_data, measureset_data, piping_data, stbi_data
):
    ...
    for count, section in vakindeling_gdf.iterrows():
        # STBI
        stbi_data.set_index("DOORSNEDE").loc[section.DSN_STBI].to_csv(
            working_dir.joinpath("output", "StabilityInner", section.DSN_STBI + ".csv"),
            header=False,
        )

        # Piping
        piping_data.set_index("dwarsprofiel").loc[section.DSN_STPH].drop(
            columns=["Naam traject"]
        ).transpose().to_csv(
            working_dir.joinpath("output", "Piping", str(section.DSN_STPH) + ".csv"),
            header=False,
        )
        ...
        shutil.copyfile(
            ...
            working_dir.joinpath(
                "output", "Overflow", "2100", section.DSN_GEKB + ".txt"
            ),
        )
        shutil.copyfile(
            working_dir.joinpath(
            ...
            working_dir.joinpath(
                "output", "Overflow", "2025", section.DSN_GEKB + ".txt"
            ),
        )
        shutil.copyfile(
            working_dir.joinpath(
                ...
                "Resultaten_W2100",
                section.DSN_GEKB,
                "DESIGNTABLE_{}.txt".format(section.DSN_GEKB),
            ),
            working_dir.joinpath(
                "output",
                "Waterstand",
                "2100",
                "Waterstand_{}.txt".format(section.DSN_GEKB),
            ),
        )
        shutil.copyfile(
            working_dir.joinpath(
...
def write_tool_input_legacy(working_dir):
    ...
    vakindeling_gdf = gpd.read_file(
        working_dir.joinpath("intermediate", "Vakindeling_Veiligheidsrendement.shp")
    )
    housing_data = pd.read_csv(
        working_dir.joinpath("intermediate", "Bebouwing", "Bebouwing_data.csv"),
        index_col=0,
    )
    # NB: no customs here, should be extended.
    measureset_data = pd.read_csv(
        working_dir.joinpath("intermediate", "Maatregelen", "base_measures.csv"),
        delimiter=",",
    )

    piping_data = pd.read_csv(
        working_dir.joinpath("intermediate", "Piping", "Piping_data.csv"), index_col=0
    )
    stbi_data = pd.read_csv(
        working_dir.joinpath("intermediate", "STBI", "STBI_data.csv"), index_col=0
    )
    gekb_data = pd.read_csv(
        working_dir.joinpath("intermediate", "Overslag", "GEKB_data.csv"), index_col=0
    )

    # Make subfolders if not exist:
    if not working_dir.joinpath("output", "StabilityInner").is_dir():
        working_dir.joinpath("output", "StabilityInner").mkdir(
    ...
    # write section files:
    write_dike_section_files_legacy(
        working_dir, vakindeling_gdf, housing_data, measureset_data, gekb_data
    )

    write_mechanism_data_legacy(
        working_dir,
        vakindeling_gdf,
        housing_data,
        measureset_data,
...
if __name__ == "__main__":
    ...
    working_dir = Path(
        r"c:\Users\klerk_wj\OneDrive - Stichting Deltares\00_Projecten\11_VR_Prioritering WSRL\Gegevens 38-1"
    )
