...
def vka_measures(measures_basis, measures_custom_all, measures_location, path, traject):
    ...
    vka_cols = [
        "voorkeursalternatief_1",
        "voorkeursalternatief_2",
        "voorkeursalternatief_3",
    ]
    ...
    for i, measures_custom_vka in vkas.iterrows():
        ...
        # write custom to csv
        measure_custom_1 = measures_custom_all[
            measures_custom_all["variantnaam"] == measures_custom_vka.Value
        ]
        naam = path.joinpath(
            traject, "Output/Measures", measures_custom_vka.Value + ".csv"
        )
        measure_custom_1.to_csv(naam, index=False)
        measures_basis.loc[j, "year"] = int(
            measure_custom_1.year - 2025
        )  # TODO, check 2025
...
def main():
    ...
    backup_file_name = (
        file_name + ".bak"
    )  # Name for backupping the main file before making changes
    ...
    load_file = path.joinpath(
        "Crest_WL_HBN_data.csv"
    )  # File originating from the hydraulic load computations by HKV in 2018. Should be in same path as main file.
    ...
    DikeSections = (
        df["Dijkvakindeling_keuze_info"]
        .rename(columns=df["Dijkvakindeling_keuze_info"].iloc[0])
        .drop(df["Dijkvakindeling_keuze_info"].index[0])
    )
    DikeSections = DikeSections[
        (
            (DikeSections["Traject"] == traject)
            & (DikeSections["Wel of niet meerekenen"] == 1)
        )
    ...
    if "Maatregelen" in df.keys():
        ...
        measures_per_location = pd.read_csv(
            path.joinpath(traject, "Input/measures.csv"), delimiter=";"
        )
    ...
    if fill_load_values:
        ...
        HBNs = pd.read_csv(
            load_file,
            usecols=[0, 8, 9, 10, 11, 12, 13],
            header=0,
            names=["Dijkpaal", 2015, 2025, 2505, 2075, 2100, 2125],
        )
        ...
        for i in DikeSections.index:
            ...
            df_CrestHeight = crest_heights[
                crest_heights["Dijkpaal"] == DikeSections["Dwarsprofiel piping"][i][:5]
            ]
            CrestHeight = float(df_CrestHeight["Huidige Kruinhoogte [m+NAP]"])

            df_YearlyWLRise = water_levels[
                water_levels["Dijkpaal"]
                == DikeSections["Dwarsprofiel HoogteToetspeil"][i]
            ]
            YearlyWLRise_factor = float(
                (df_YearlyWLRise["2125"].values - df_YearlyWLRise["2015"].values)
                / (2125 - 2015)
            )

            df_HBNRise_factor = HBNs[
                HBNs["Dijkpaal"] == DikeSections["Dwarsprofiel HoogteToetspeil"][i]
            ]
            HBNRise_factor = (
                float(
                    (df_HBNRise_factor[2125].values - df_HBNRise_factor[2015].values)
                    / (2125 - 2015)
                )
                / YearlyWLRise_factor
            )
            ...
            for row in range(ws.max_row):
                if (
                    ws[row + 1][8].value == DikeSections["Dwarsprofiel piping"][i]
                    and ws[row + 1][5].value == 1
                ):
    ...
    if any(
        STBI_data["dwarsprofiel"].duplicated()
    ):  # or any(Piping_data['dwarsprofiel'].duplicated()):
    ...
    if not path.joinpath(traject, "Output/StabilityInner").is_dir():
        path.joinpath(traject, "Output/StabilityInner").mkdir(
        ...
        path.joinpath(traject, "Output/FragilityCurve_STBI").mkdir(
            parents=True, exist_ok=True
        )
    ...
    for i in DikeSections.index:
        ...
        STBI_data_location = (
            STBI_data[
                STBI_data["dwarsprofiel"] == DikeSections["Dwarsprofiel STBI/STBU"][i]
            ]
            .transpose()
            .drop(["dwarsprofiel"], axis=0)
            .reset_index()
        )
        ...
        if (
            False
            if pd.isnull(STBI_data_location.loc["FragilityCurve", "Value"])
            else True
        ):
            General["Type"] = [
                "",
            ...
            src = path.joinpath(
                traject,
                "Input/FragilityCurve_STBI",
                STBI_data_location.loc["FragilityCurve", "Value"],
            )
            dst = path.joinpath(
                traject,
                "Output/FragilityCurve_STBI",
                STBI_data_location.loc["FragilityCurve", "Value"],
            )
            ...
            if STBI_data_location["Value"][[1, 2, 5, 6]].isnull().values.all():
                raise Exception(
                    "STBI data of cross-section {} (Dike section {}) contains NaN values".format(
                        DikeSections["Dwarsprofiel STBI/STBU"][i],
                        DikeSections["dv_nummer"][i],
                    )
                )
                sys.exit()
        STBI_data_location.to_csv(
            path.joinpath(
                traject,
                "Output/StabilityInner",
                DikeSections["Dwarsprofiel STBI/STBU"][i] + "_StabilityInner.csv",
            )
        )

        # Then for piping:
        Piping_data_location = (
            Piping_data[
                Piping_data["dwarsprofiel"] == DikeSections["Dwarsprofiel piping"][i]
            ]
            .transpose()
            .drop(["dwarsprofiel"], axis=0)
            .reset_index()
        ...
        if Piping_data_location.iloc[:, 1].isnull().values.any():
            raise Exception(
                "Piping data of cross-section {} (Dike section {}) contains NaN values".format(
                    DikeSections["Dwarsprofiel STBI/STBU"][i],
                    DikeSections["dv_nummer"][i],
                )
            )
        ...
        Piping_data_location.to_csv(
            path.joinpath(
                traject,
                "Output/Piping",
                DikeSections["Dwarsprofiel piping"][i] + "_Piping.csv",
            )
        )

        # Then we read and write data for overflow (this is a bit more complicated):
        HBN_basis = pd.read_csv(
            path.joinpath(traject, "Input/base_HBN.csv"), delimiter=";"
        )
        OverflowData = readDesignTable(
            path.joinpath(
                traject,
                "Input/designtables_HBN/DESIGNTABLE_"
                + DikeSections["Dwarsprofiel HoogteToetspeil"][i]
                + ".txt",
            )
        )
        ...
        if (
            not overflow_target_beta
        ):  # This is the common variant. Quite straightforward
            ...
            beta_t = (
                beta_t_overflow[1] - beta_t_overflow[0]
            ) * np.random.random_sample(1) + beta_t_overflow[0]
            crestlevel = fsolve(
                OverflowSimple,
                DikeSections["Kruinhoogte"][i],
                args=(
                    5,
                    OverflowData["Value"].values,
                    np.ones((len(OverflowData["Value"].values), 1)) * 5,
                    OverflowData["Beta\n"].values,
                    "assessment",
                    None,
                    None,
            ...
            print(
                "for section "
                + str(DikeSections.iloc[i]["dv_nummer"])
                + " changed crest from "
                + str(DikeSections["Kruinhoogte"][i])
                + " to "
                + str(crestlevel)
            )
        ...
        if len(DikeSections["Kruindaling"].value_counts()) > 0:
            ...
            HBN_basis["dhc(t)"].iloc[0] = 0.0
        HBN_basis.to_csv(
            path.joinpath(
                traject,
                "Output/Overflow",
                DikeSections["Dwarsprofiel HoogteToetspeil"][i] + "_Overflow.csv",
            ),
            index=False,
        )
        ...
        copyfile(
            path.joinpath(
                traject,
                "Input/designtables_TP/DESIGNTABLE_"
                + DikeSections["Dwarsprofiel HoogteToetspeil"][i]
                + ".txt",
            ),
            path.joinpath(
                traject,
                "Output/Toetspeil/DESIGNTABLE_"
                + DikeSections["Dwarsprofiel HoogteToetspeil"][i]
                + ".txt",
            ),
        )
        ...
        if "Maatregelen" in df.keys():
            ...
            measures_location = (
                Measures[Measures["Naam dijkvak"] == DikeSections["dv_nummer"][i]]
                .transpose()
                .drop(["Naam dijkvak"], axis=0)
                .reset_index()
            )
            measures_location.columns = ["Name", "Value"]
            measures_location = measures_location.set_index("Name")
            # read tables with measures
            measures_basis = pd.read_csv(
                path.joinpath(
                    traject,
                    "Input/measures",
                    measures_location.loc["Basismaatregelen"].Value,
                ),
                delimiter=",",
            )
            measures_custom_all = pd.read_csv(
                path.joinpath(traject, "Input/measures/custom_measures.csv")
            )
            vka_cols = [
                "voorkeursalternatief_1",
                "voorkeursalternatief_2",
            ...
            if not all(pd.isna(measures_location.loc[vka_cols].Value)):
                measures_per_location = vka_measures(
                    measures_basis,
                    measures_custom_all,
                    measures_location,
                    path,
        ...
        profile = pd.read_csv(
            path.joinpath(
                traject,
                "Input/profiles",
                DikeSections["Dwarsprofiel Geometrie"][i] + ".csv",
            ),
            index_col=0,
        )
        ...
        if len(profile) == 6:
            ...
            type = pd.DataFrame(
                ["innertoe", "innercrest", "outercrest", "outertoe"], columns=["type"]
            )
            profile["type"] = type
        else:
        ...
        # Fill houses tab
        houses_data_location = (
            Housing[
                (
                    (Housing["Naam dijkvak"] == DikeSections["dv_nummer"][i])
                    & (Housing["Naam traject"] == traject)
                )
            ]
            .transpose()
            .drop(["Naam traject", "Naam dijkvak"], axis=0)
            .reset_index()
        )
        ...
        try:
            writer = pd.ExcelWriter(
                path.joinpath(
                    traject,
                    "Output/DV"
                    + "{:02d}".format(DikeSections["dv_nummer"][i])
                    + ".xlsx",
                )
            )
        except:
            writer = pd.ExcelWriter(
                path.joinpath(
                    traject, "Output/DV" + DikeSections["dv_nummer"][i] + ".xlsx"
                )
            )
