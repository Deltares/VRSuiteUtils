...
def run():
    ...
    traject.generate_cross_section(cross_section_distance=10000, # distance between cross sections
                                   foreshore_distance=30,
    ...
    with open(output_path.joinpath('traject_profiles2.csv'), 'w', newline='') as csvfile:
        writer = csv.writer(csvfile, delimiter=',')
        count = 0
        foreshore_length = 30
        for profile in traject.profiles:
            if count == 0:
                header = ['index'] + list(np.round(profile[:, 3], 1)-foreshore_length)
                writer.writerow(header)
                row = [count] + list(np.round(profile[:, 2], 1))
                writer.writerow(row)
                count += 1
            else:
                row = [count] + list(np.round(profile[:, 2], 1))
