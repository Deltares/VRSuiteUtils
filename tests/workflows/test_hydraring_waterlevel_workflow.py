...
def test_hydraring_waterlevel_workflow(project_folder:str,  request: pytest.FixtureRequest):
    ...
    api.generate_and_evaluate_waterlevel_computations(test_data.joinpath(project_folder, "preprocessor.config"), _output_path)

    #get the relative path from the config
    _reference_dir = read_config_file(test_data.joinpath(project_folder, "preprocessor.config"), ['output_map_waterstand'])['output_map_waterstand']
