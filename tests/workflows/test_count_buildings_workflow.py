...
@pytest.mark.skip(reason="Test takes to long to run in TeamCity")
@pytest.mark.parametrize("project_folder",
                         [pytest.param("31-1_v2", id = '31-1')])
def test_count_buildings_workflow(project_folder:str,  request: pytest.FixtureRequest):
    ...
    api.count_buildings(test_data.joinpath(project_folder, "preprocessor.config"), _output_path)

    #compare selected_profiles.csv in both dirs
    for file in _output_path.rglob("building_count*.csv"):
