...
def plot_lcc(Strategies, traject, input_path=False, fig_size=(12, 2), flip=False, title_in=False, subfig=False, greedymode ='Optimal', color = False):
    ...
    plt.savefig(input_path.joinpath('LCC.png'),dpi=300, bbox_inches='tight', format='png')

def main():
    #import settings of run:
    directory = Path(r'n:\Projects\11208000\11208392\C. Report - advise\Gegevens 16-1\rerun\results')
    ...
    AllStrategies[1].make_solution(directory.joinpath('results', 'FinalMeasures_Doorsnede-eisen.csv'), type='Final')
    AllStrategies[0].make_solution(directory.joinpath('results', 'FinalMeasures_Veiligheidsrendement.csv'),step = AllStrategies[0].SafetyStandardStep, type='SatisfiedStandard')
    ...
    case_settings = {'directory':directory,
                     'language':'NL',
                     'beta_or_prob':'beta'}
    for plot_t in [0, 20, 50]:
        plot_year = str(plot_t+t_0)
        TestCase.plot_assessment(fig_size=figsize, t_list=[plot_t], labels_limited=True, system_rel=True, show_xticks=True,case_settings = case_settings,
        ...
        TestCase.plot_assessment(fig_size=figsize, t_list=[plot_t], labels_limited=True,system_rel=True, case_settings = case_settings,
    ...
    AllStrategies[0].plot_measures(traject=TestCase,input_path=directory,fig_size=figsize,crestscale=25.,
    ...
    #
    AllStrategies[1].plot_measures(traject=TestCase,input_path=directory,fig_size=figsize,crestscale=25.,
    ...
    plot_lcc(AllStrategies, TestCase, input_path=directory, fig_size=figsize, flip=True, greedymode=greedy_mode,
    ...
    for t_plot in [0,50]:
        for cost_type in ['Initial', 'LCC']:
            MeasureTable = StrategyBase.get_measure_table(AllSolutions, language = 'EN',abbrev=True)
            figsize = (6,4)
            plt.figure(102, figsize=figsize)
            AllStrategies[0].plot_beta_costs(TestCase, save_dir= directory,t=t_plot, cost_type=cost_type,
            ...
            AllStrategies[1].plot_beta_costs(TestCase, save_dir= directory,t=t_plot, cost_type=cost_type,
            ...
            plt.savefig(directory.joinpath('Priority order Beta vs LCC_' + str(t_plot+t_0) + '.png'),dpi=300,bbox_inches='tight',format='png')
            plt.close()
